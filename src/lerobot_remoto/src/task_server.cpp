...
#include "lerobot_msgs/action/lerobot_task.hpp"
...
public:
    ...
    : Node("task_server", options)
  {
    ...
    action_server_ = rclcpp_action::create_server<lerobot_msgs::action::LerobotTask>(
...
private:
  rclcpp_action::Server<lerobot_msgs::action::LerobotTask>::SharedPtr action_server_;
  ...
  rclcpp_action::GoalResponse goalCallback(
      ...
      std::shared_ptr<const lerobot_msgs::action::LerobotTask::Goal> goal)
  {
  ...
  rclcpp_action::CancelResponse cancelCallback(
      const std::shared_ptr<rclcpp_action::ServerGoalHandle<lerobot_msgs::action::LerobotTask>> goal_handle)
  {
    (void)goal_handle;
  ...
  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<lerobot_msgs::action::LerobotTask>> goal_handle)
  {
    RCLCPP_INFO(get_logger(), "Executing goal");
    auto result = std::make_shared<lerobot_msgs::action::LerobotTask::Result>();
    ...
    {
      arm_joint_goal_ = {0.0, 1.57, -1.57, 0.0, -1.57};
      gripper_joint_goal_ = {-1.57};
    }
    else if (goal_handle->get_goal()->task_number == 2)
    {
      arm_joint_goal_ = {1.57, 1.57, -1.57, 0.0, 0};
      gripper_joint_goal_ = {0.0};
    }
    else
    {
...
}  // namespace lerobot_remote

RCLCPP_COMPONENTS_REGISTER_NODE(lerobot_remoto::TaskServer)